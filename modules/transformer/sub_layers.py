...
class  MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        """
        Args:
            q: [batch_size, len_q, transformer_size]
            k: [batch_size, len_k, transformer_size]
            v: [batch_size, len_v, transformer_size]
        ...
        q = self.q_linear(q).view(sz_b, len_q, self.num_heads, self.k_size) # [batch_size, len_q, num_heads, k_size]
        k = self.k_linear(k).view(sz_b, len_k, self.num_heads, self.k_size) # [batch_size, len_k, num_heads, k_size]
        v = self.v_linear(v).view(sz_b, len_v, self.num_heads, self.v_size) # [batch_size, len_v, num_heads, v_size]
        #  print('q size: ', q.shape) 
        #  print('k size: ', k.shape)
        #  print('v size: ', v.shape)

        q = q.permute(2, 0, 1, 3).contiguous().view(-1, len_q, self.k_size) # [num_heads * batch_size, len_q, k_size]
