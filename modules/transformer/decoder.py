...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embedding = nn.Embedding.from_pretrained(
            get_sinusoid_encoding_table(
                n_position, config.embedding_size, padid=PAD_ID),
            freeze=True
        )

        self.layer_stack = nn.ModuleList([
            DecoderLayer(config) for _ in range(config.t_num_layers)]
        )
    ...
    def forward(self,
        ...
        # -- Prepare masks
        non_pad_mask = get_non_pad_mask(dec_inputs, PAD_ID)

        slf_attn_mask_subseq = get_subsequent_mask(dec_inputs)

        slf_attn_mask_keypad = get_attn_key_pad_mask(dec_inputs, dec_inputs, PAD_ID)

        slf_attn_mask = (slf_attn_mask_keypad + slf_attn_mask_subseq).gt(0)

        dec_enc_attn_mask = get_attn_key_pad_mask(enc_inputs, dec_inputs, PAD_ID)
